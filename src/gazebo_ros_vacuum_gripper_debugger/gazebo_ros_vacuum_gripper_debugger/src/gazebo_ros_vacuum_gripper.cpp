...
{
  ROS_INFO_NAMED("vacuum_gripper", "Loading gazebo_ros_vacuum_gripper_debugger");
  ...
  {
    ROS_FATAL_NAMED("vacuum_gripper", "gazebo_ros_vacuum_gripper_debugger plugin missing <bodyName>, cannot proceed");
    return;
  }
  else
  ...
  if (!link_)
  {
    ...
    ROS_FATAL_NAMED("vacuum_gripper", "gazebo_ros_vacuum_gripper_debugger plugin error: link named: %s does not exist", link_name_.c_str());
    ROS_FATAL_NAMED("vacuum_gripper", "gazebo_ros_vacuum_gripper_debugger plugin error: You should check it exists and is not connected with fixed joint");
    ROS_FATAL_NAMED("vacuum_gripper", "gazebo_ros_vacuum_gripper_debugger plugin error: Found links are: %s", found.c_str());
    return;
  }

  if (!_sdf->HasElement("topicName"))
  {
    ROS_FATAL_NAMED("vacuum_gripper", "gazebo_ros_vacuum_gripper_debugger plugin missing <serviceName>, cannot proceed");
    return;
  }
  else
  ...
  {
    ROS_FATAL_NAMED("vacuum_gripper", "gazebo_ros_vacuum_gripper_debugger plugin missing <maxDistance>, cannot proceed");
    return;
  }
  else
  ...
  {
    ROS_FATAL_NAMED("vacuum_gripper", "gazebo_ros_vacuum_gripper_debugger plugin missing <minDistance>, cannot proceed");
    return;
  }
  else
  ...
  {
    ROS_FATAL_NAMED("vacuum_gripper", "gazebo_ros_vacuum_gripper_debugger plugin missing <maxForce>, cannot proceed");
    return;
  }
  else
  ...
  ROS_INFO_NAMED("vacuum_gripper", "Loaded gazebo_ros_vacuum_gripper_debugger");
}
...
{
  if (status_) {
    ROS_WARN_NAMED("vacuum_gripper", "gazebo_ros_vacuum_gripper_debugger: already status is 'on'");
  } else {
    status_ = true;
    ROS_INFO_NAMED("vacuum_gripper", "gazebo_ros_vacuum_gripper_debugger: status: off -> on");
  }
  return true;
}
                                     ...
                                     std_srvs::Empty::Response &res)
{
  if (status_) {
    status_ = false;
    ROS_INFO_NAMED("vacuum_gripper", "gazebo_ros_vacuum_gripper_debugger: status: on -> off");
  } else {
    ROS_WARN_NAMED("vacuum_gripper", "gazebo_ros_vacuum_gripper_debugger: already status is 'off'");
  }
  return true;
}
...
void GazeboRosVacuumGripper::UpdateChild()
{
  ...
  for (size_t i = 0; i < models.size(); i++) {
    ...
    for (size_t j = 0; j < links.size(); j++) {
      ...
      // Only log when object is close to gripper
      if (norm < max_distance_) {
        ROS_DEBUG_NAMED("vacuum_gripper", "gazebo_ros_vacuum_gripper_debugger: model: %s, link: %s, norm: %f", models[i]->GetName().c_str(), links[j]->GetName().c_str(), norm);
      }
      if (norm < max_distance_) {
#if GAZEBO_MAJOR_VERSION >= 8
...
#endif
        double norm_force = 10 / norm;
        if (norm < max_distance_) {
          // apply friction like force - lock position when very close
          link_pose.Set(parent_pose.Pos(), link_pose.Rot());
          links[j]->SetWorldPose(link_pose);
          // Also synchronize velocities for better adhesion
          links[j]->SetLinearVel(link_->WorldLinearVel());
          links[j]->SetAngularVel(link_->WorldAngularVel());
        }
        if (norm_force > 10) {
